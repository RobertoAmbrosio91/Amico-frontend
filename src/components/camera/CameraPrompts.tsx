...
import { uploadMediaTos3WithSource } from "@/services/uploadMediaTos3 ";
...
import FadeOutView from "../animated/FadeOutView";
import createPrompt from "@/hooks/events/prompts";
...
const CameraPrompts: FC<CameraComponentProps> = ({
  setIsCameraOpen,
  eventId,
  ...
  const [typeOfMemory, setTypeOfMemory] = useState<string>("memory");
      ...
      if (cloudfrontURL) {
        try {
          const createMemoryResponse = await createMemory(
            ...
            caption,
            currentUser.token,
            isPromptReply ? promptName : undefined
          );
          if (createMemoryResponse && createMemoryResponse.success) {
            if (typeOfMemory !== "memory") {
              const create_prompt = await createPrompt(
                currentUser.token,
                eventId,
                prompt,
                createMemoryResponse.result._id,
                cloudfrontURL
              );
            }
            setIsLoading(false);
  ...
  const message = () => {
    if (isPromptReply) {
      ...
      return `Make ${eventData?.name} unforgettable, share the bestmemories with your friends!`;
    }
  };
  const disabledCreate =
    typeOfMemory != "memory" && prompt.length < 5 ? true : false;
        ...
        {!capturedMediaUri && !selectedMediaUri && (
          ...
          >
            <Pressable style={styles.cameraContainer} onPress={toggleFocusMode}>
              ...
              {/* {messageVisible && ( */}
              <FadeOutView isVisible={messageVisible}>
                <View style={styles.eventTextContainer}>
                  <Text style={styles.eventText}>{message()}</Text>
                </View>
              </FadeOutView>
              {/* )} */}
              ...
              {!isPromptReply && (
                <CameraOptions
                  ...
                  setTypeOfMemory={setTypeOfMemory}
                />
              )}
            </Pressable>
          </Camera>
            ...
            >
              ...
              >
                <Feather
                  name="send"
                  size={23}
                  color={disabledCreate ? "rgba(256,2256,256,0.3)" : "white"}
                />
              </TouchableOpacity>
            </Animated.View>
          </View>
